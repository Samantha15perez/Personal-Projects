...
--------------------------------
insert into Activities (leadid, Activitydate, complete, modifieddate)
values 
('1', '4/16/2018', 1, '4/16/2018'),
('4', '5/13/2017', 1, '5/13/2017')
...
('11/23/2017', 'Web Developer', 'If you are interested in joining a company that pushes the envelope in the advertising industry and definitely cares about providing a great working environment for its employees, then apply immediately.', 'full-time', 'Ocala', (Select Companyid from Companies where Companyname = 'CyberCoders'), 1, (select sourceid from sources where sourcename = 'monster.com'))
 ...
 update contacts
set active = 1
where companyid = 'Exeter Employment'


  update Contacts
 set active = 1
 where courtesytitle = 'Mr.'

Print '**MOCK DATA ENTERED**'

go
create trigger trg_UpdateModDate
on leads
AFTER UPDATE
as
UPDATE leads
set modifieddate = getdate()
where leadid in (select distinct leadid from inserted) 



update leads
set description = 'test'
where leadid in (2, 3, 7)
...
go
create proc JobLeadsPerDay (@FromDate date, @Untildate date)
as
BEGIN
Select recorddate, count(Recorddate) from Leads 
where RecordDate >= @FromDate
AND Recorddate <= @UntilDate
GROUP BY recorddate
END

exec JobLeadsPerDay @fromdate = '2/10/2012', @Untildate = '4/18/2018'

--SP2

go
create proc InactiveLeads
as
BEGIN
Select * from Leads 
where Leadopen = 1
AND modifieddate < dateadd(day, -7, getdate())
END
...
as
BEGIN
Select sourceid, count(leadid) as [# of Leads] from Leads
GROUP BY sourceid
...
on C.Companyid = A.Agencyname
inner join Companies CC
on CC.Companyname = C.Companyid
where C.active = 1
END

exec ActiveContactList


--SP5

go
create proc ActiveLeads
as
Begin
select jobtitle, CC.Companyid, L.[description], location, Concat(Courtesytitle, ' ', ContactFirstname, ' ', contactlastname) as ContactName, CC.Phone, DATEDIFF(day, modifieddate, getdate()) as DaysSinceLastActive from leads L
...
go
create proc SearchLog (@FromDate date, @Untildate date)
as
BEGIN
Select Activitydate, activitytype, Jobtitle, CC.Companyid, complete from activities A
inner join Leads L 
...
inner join Contacts CC
on CC.Companyid = C.Companyname
where A.modifieddate >= @FromDate
AND A.modifieddate <= @UntilDate
END

exec SearchLog @Fromdate = '2/1/2012', @Untildate = '4/18/18'
...
as
begin
select Leadid, CC.companyid, A.Agencyname, Concat(Courtesytitle, ' ', ContactFirstname, ' ', contactlastname) as ContactName, CC.Phone, CC.email, S.Sourcename from leads L
inner join Companies C
on C.CompanyID = L.Companyid
inner join Contacts CC
on CC.Companyid = C.Companyname
